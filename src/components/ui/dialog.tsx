  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-white data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-neutral-500 p-6 shadow-lg duration-200 sm:max-w-lg",
          className
        )}
        {...props}
      >
        ...
        <DialogPrimitive.Close className="ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute top-2 right-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4">
          <div
            className={"border border-danger rounded-full p-2 cursor-pointer"}
          >
            <XIcon size={30} className={"text-danger"} />
          </div>
...
function DialogTitle({
  ...
  return (
    <DialogPrimitive.Title
      ...
      className={cn(
        "text-base md:text-lg leading-none font-semibold text-left w-[80%]",
        className
      )}
      {...props}
    />
